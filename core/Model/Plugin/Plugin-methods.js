...
model.Plugin.methods.LoadCommonJS = function(fileName) {
    ...
    console.log('fileName %s, \n require(fileName) %o', fileName, require(fileName).taruInfo);
	...
	*/
    ...
    if (!existingPlugin) {
        ...
        if (uploadedFile) {
            ...
            try {
                uploadedFile.moveTo(FileSystemSync("Modules").path + uploadedFile.nameNoExt + '.js');
            ...
            try {
                ...
                procesState.error = err;
                procesState.add('intall failed: ' + fileName);
            }
        }
...
model.Plugin.entityMethods.Module = function() {
    ...
    if (!application.ActivePlugins || !application.ActivePlugins[this.fileName]) {
    	...
    	application.ActivePlugins[this.fileName]
    }
...
model.Plugin.methods.ListAdaptersProtocols = function() {
...
model.Plugin.entityMethods.GenerateWorkers = function() {
    ...
    var exisitingWorker = this.workers.find('protocol == :1', this.taruInfo.connection.protocol)
    if (exisitingWorker) {
