...
exports.taruConnect =  function (config, connectionAdapter) {
	...
	if (!config.step2 || !config.step3 || !config.step4) {
		...
		;
		config.connection = {};
		config.connection['getExternalAuth'] = URLText;
		return {
			newCongfig: config
		}
		var step2Results = requestAuthorizationCode (redirectUri); 
...
exports.StartWorker = function (endPointEntity) {
	...
	addHttpRequestHandler('(?i)^/' + endPointEntity.config.uri + '$', 'Modules/smartthings.js', 'main');
...
exports.StopWorker = function (endPointEntity) {
	...
	removeHttpRequestHandler('(?i)^/' + endPointEntity.ID + '$', 'Modules/smartthings.js', 'main');
...
function main(request,response) {
	...
	/*
	handle step 2add Oauth token response
